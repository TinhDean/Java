...
public static double tongDoanhThu() {
    ...
    try {
        String sql = "SELECT SUM(TongTien) AS TotalRevenue FROM HoaDon WHERE TrangThai = N'Đã thanh toán'";
        ResultSet rs = provider.executeQuery(sql);
...
public static DefaultTableModel locHoaDonTheoMaKhachHang(int maKhachHang) {
    DefaultTableModel model = new DefaultTableModel();
    // Define columns for the table model
    model.addColumn("Mã Hóa Đơn");
    model.addColumn("Mã Khách Hàng");
